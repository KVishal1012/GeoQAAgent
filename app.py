...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--diagnose-config", action="store_true", help="Print the effective runtime configuration and exit.")
    ...
    parser.add_argument("--agent-run", action="store_true", help="Run the V4 tool-using analyst agent after deterministic QA artifacts are written.")
    parser.add_argument("--agent-task", default="report", choices=["report", "fix_plan", "handoff"], help="V4 agent task to execute.")
    parser.add_argument("--agent-max-steps", type=int, default=None, help="Optional maximum number of internal tool steps for the V4 agent session.")
    parser.add_argument("--llm-provider", default="openai", choices=["openai", "static"], help="LLM provider for agent runs. Use 'static' for the no-API-key demo path.")
    parser.add_argument("--llm-model", default=None, help="Optional LLM model override for agent runs.")
    parser.add_argument("--approve-agent-report", action="store_true", help="Approve the generated agent report when consistency checks pass.")
    parser.add_argument("--reject-agent-report", action="store_true", help="Reject the generated or existing agent report draft and record reviewer metadata.")
    parser.add_argument("--reviewer-name", default=None, help="Reviewer name required for agent report approval.")
    parser.add_argument("--review-notes", default=None, help="Optional notes recorded with agent report approval or rejection.")
    parser.add_argument("--review-output-dir", default=None, help="Existing run output directory to approve or reject without regenerating artifacts.")
    parser.add_argument("--compare-run-dir", default=None, help="Base run output directory used to compare against the current or target run output directory.")
    parser.add_argument("--target-run-dir", default=None, help="Target run output directory used with --compare-run-dir for run comparison.")
    parser.add_argument("--generate-fix-plan", action="store_true", help="Generate fix plan artifacts for an existing run output directory.")
    parser.add_argument("--export-handoff-bundle", action="store_true", help="Export a handoff bundle for an existing run output directory.")
    parser.add_argument("--comparison-key", default=None, help="Optional comparison key to include when exporting a handoff bundle.")
...
def validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    if args.review_output_dir and agent_requested:
        parser.error("--review-output-dir cannot be combined with agent generation")
    if args.review_output_dir and not (review_action_requested or existing_run_action_requested):
        parser.error("--review-output-dir requires a review action, --generate-fix-plan, or --export-handoff-bundle")
    if review_action_requested and not args.review_output_dir and not agent_requested:
        parser.error("review actions require either an agent generation flag or --review-output-dir")
    if args.llm_provider == "static" and agent_requested and not args.static_report_file:
    ...
    try:
        ...
        if args.review_output_dir:
            ...
            if args.approve_agent_report or args.reject_agent_report:
                ...
                workflow_result.update(review_existing_agent_report(args.review_output_dir, action=action, reviewer_name=args.reviewer_name or "", notes=args.review_notes))
            if args.generate_fix_plan:
                workflow_result.update(generate_fix_plan_artifacts(args.review_output_dir, playbook_dir=args.playbook_dir))
            if args.export_handoff_bundle:
                workflow_result.update(export_handoff_bundle(args.review_output_dir, comparison_key=args.comparison_key))
        ...
        if agent_requested:
            ...
            if args.llm_provider == "static":
                ...
                gateway = build_openai_gateway(config, default_model=runtime_model)

            agent_task = "report" if args.agent_report and not args.agent_run else args.agent_task
            agent_artifacts = generate_agent_report_artifacts(
                ...
                task=agent_task,
                max_steps=args.agent_max_steps or config.agent_max_steps,
                comparison_key=args.comparison_key,
            )
            result.artifact_paths.update(agent_artifacts)
            if args.reject_agent_report:
                review_result = review_existing_agent_report(result.artifact_paths["output_dir"], action="reject", reviewer_name=args.reviewer_name or "", notes=args.review_notes)
